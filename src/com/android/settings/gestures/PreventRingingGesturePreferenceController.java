...
public class PreventRingingGesturePreferenceController extends AbstractPreferenceController
    ...
    public boolean onPreferenceChange(Preference preference, Object newValue) {
        ...
        if (isAdd) {
            if (currentValue.get(0).equals(ZAP_VOLUME_HUSH_OFF))
                currentValue.clear();
            if (!currentValue.contains(preventRingingSetting))
                currentValue.add(preventRingingSetting);
        } else {
            if (currentValue.size() == 1 ||
                    preventRingingSetting.equals(ZAP_VOLUME_HUSH_OFF)) {
                currentValue.clear();
                currentValue.add(ZAP_VOLUME_HUSH_OFF);
    ...
    public void updateState(Preference preference) {
        ...
        final boolean isVibrate = enabled && preventRingingSetting.contains(ZAP_VOLUME_HUSH_VIBRATE);
        final boolean isMute = enabled && preventRingingSetting.contains(ZAP_VOLUME_HUSH_MUTE);
        final boolean isNormal = enabled && preventRingingSetting.contains(ZAP_VOLUME_HUSH_NORMAL);
    ...
    private String keyToSetting(String key) {
        switch (key) {
            case KEY_MUTE:
                return ZAP_VOLUME_HUSH_MUTE;
            case KEY_VIBRATE:
                return ZAP_VOLUME_HUSH_VIBRATE;
            case KEY_NORMAL:
                return ZAP_VOLUME_HUSH_NORMAL;
            default:
                return ZAP_VOLUME_HUSH_OFF;
        }
    }
